...
Start by reading AGENTS.md, the {w["wave_id"]} shared docs, and required context. Run:

```bash
python3 scripts/agent/wave_task_preflight.py {preflight_args}
```
...
Write test evidence to `{w["local_run_dir"]}/{task_id}/tests.json` and the committed handoff to `{handoff_path}` using `{w["handoff_template"]}`. Runtime `.agent-runs/{wave_id}/{task_id}/handoff.md` files are generated mirrors, not authoritative handoffs. {authority} Stop and escalate if frozen contracts or another lane must change.
"""
