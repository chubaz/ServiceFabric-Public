...
def build(root,outcome="known_committed"):
 ...
 service=GovernanceOperationsService(evaluator=VersionedPolicyEvaluator((bundle,)),approvals=ApprovalService(),operations=DurableOperationStore(Path(root)/"operations"),idempotency=IdempotencyRepository(Path(root)/"idempotency"),reconciliation=ReconciliationService(DeterministicEffectAdapter({"provider-op-1":outcome})),audit_records=ImmutableRecordRepository(Path(root)/"audit"))
...
class V3VerticalSliceTests(unittest.TestCase):
 def test_approval_durable_restart_duplicate_success_and_effect_receipt(self):
  with tempfile.TemporaryDirectory() as root:
   ...
   restarted=build(root);recovered,version=restarted.get_operation("operation-1");self.assertEqual((recovered.spec.state,version),("succeeded",5));self.assertEqual(len(restarted.list_operation_events("operation-1")),5);self.assertEqual(len(restarted.effect_receipts("operation-1")),1)
