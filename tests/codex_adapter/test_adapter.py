...
for source in reversed((
    ...
    ROOT / "packages" / "servicefabric_codex_adapter" / "src",
)):
...
class CodexAdapterTests(unittest.TestCase):
    ...
    def test_build_argv_is_data_only_and_accepts_model_options(self) -> None:
        request = self.request.model_copy(update={"model": "gpt-5-codex", "maximum_turns": 3})
        self.assertEqual(
            self.adapter.build_argv(request),
            ("codex", "exec", "--json", "--model", "gpt-5-codex", "--max-turns", "3", "implement this"),
        )
    ...
    def test_recover_result_never_manages_a_process(self) -> None:
        handle = ProviderRunHandle(provider_id="codex", run_id="run-1", task_id="task-1", state="running")
