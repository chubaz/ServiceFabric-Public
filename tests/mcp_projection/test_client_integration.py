...
from servicefabric_mcp_projection import DiscoveryService,McpCallRequest,McpClientCapabilities,ProjectedMcpTool,ProjectionCandidate,SessionManager
...
def initialize(value):return value.initialize(session_id="session-1",caller=CallerContext(subject_ref="user-alice",principal_type="human",tenant_ref="tenant-demo",issuer="identity",scopes=("math-calculate",),authentication_strength="multi_factor"),adapter_ref="trusted-mcp-adapter",capabilities=McpClientCapabilities(structured_results=True),now=NOW)
