import unittest
from test_governance_integration import GovernanceIntegrationTests
...
class GovernedInvocations:
 def __init__(self):self.requests=[]
 def invoke(self,request,**kwargs):
  self.requests.append((request,kwargs));return ToolResult(apiVersion="servicefabric.ai/v1alpha1",kind="ToolResult",status="success",invocation_id=request.spec.request_id,tool_id="math.calculate",revision_ref="1.0.0",started_at=NOW,completed_at=NOW,duration=timedelta(),data={"value":2})
class Operations:
 def get_operation(self,operation_ref):raise AssertionError("task inspection was not requested")
def client():
 candidate=ProjectionCandidate("math.calculate","1.0.0","math-calculate","Calculate","Calculate.",{},True,True,("math-calculate",),structured_result=True);invocations=GovernedInvocations();tool=ProjectedMcpTool(name="math-calculate",canonical_tool_id="math.calculate",revision_ref="1.0.0",title="Calculate",description="Calculate.",input_schema={},structured_result=True);gateway=McpGatewayService(sessions=SessionManager(),discovery=DiscoveryService((candidate,)),tools=(tool,),governed_invocations=invocations,operations=Operations());return McpGatewayClient(gateway),invocations
...
class ClientIntegrationTests(unittest.TestCase):
 ...
 def test_client_delegates_discovery_and_call(self):
  value,invocations=client();initialize(value);self.assertEqual(value.list_tools(session_id="session-1",now=NOW).tools[0].name,"math-calculate");digest="sha256:"+"1"*64;response=value.call(session_id="session-1",now=NOW,call=McpCallRequest(request_id="request-1",tool_name="math-calculate",correlation_id="corr-1",arguments={"expression":"1+1"},idempotency_digest=digest));self.assertEqual(response.structured_content,{"value":2});self.assertEqual(invocations.requests[0][0].spec.protocol_context.protocol,"mcp");self.assertEqual(invocations.requests[0][1]["trusted_adapter_ref"],"trusted-mcp-adapter");self.assertEqual(invocations.requests[0][0].spec.idempotency.key_digest,digest)
