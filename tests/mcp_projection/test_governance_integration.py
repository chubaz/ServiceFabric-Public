...
from servicefabric_governance import ApprovalService,GovernedInvocationBoundary,InvocationGovernanceProfile,PolicyBundle,TrustedPolicyInput,VersionedPolicyEvaluator
...
def request(arguments={"value":1},approval_refs=(),scopes=("math-calculate",)):return ToolInvocationRequest.model_validate({"apiVersion":"servicefabric.ai/v1alpha1","kind":"ToolInvocationRequest","metadata":{"id":"request-1","name":"Request","description":"Fixture.","owner_ref":{"kind":"service","id":"test"}},"spec":{"request_id":"request-1","target":{"target_kind":"revision","tool_id":"math.calculate","revision_ref":"1.0.0"},"arguments":arguments,"caller_context":{"subject_ref":"user-alice","principal_type":"human","tenant_ref":"tenant-demo","issuer":"identity","scopes":scopes,"authentication_strength":"multi_factor"},"protocol_context":{"protocol":"mcp","adapter_ref":"trusted-mcp-adapter"},"budget":{},"approval_refs":approval_refs}})
...
def binding_for(value,profile_value,source,**changes):
 intent=_digest({"tool":"math.calculate","revision":"1.0.0","caller":"user-alice","arguments":source.spec.arguments});decision=value._evaluator.evaluate(TrustedPolicyInput.from_authenticated_adapter(value._policy_request(source,profile_value,intent,NOW),adapter_ref="trusted-mcp-adapter"),now=NOW);effect=decision.spec.approval_requirement.effect_refs[0]
 spec=ApprovalBindingSpec(binding_id="binding-1",approval_request_ref="request-a",approval_decision_ref="decision-a",approval_decision_digest=D,policy_decision_ref=decision.spec.decision_id,policy_version=decision.spec.policy_version,caller_ref="user-alice",tenant_ref="tenant-demo",operation_ref="operation-1",tool_id="math.calculate",revision_ref="1.0.0",intent_digest=intent,argument_digest=_digest(source.spec.arguments),effect_class=effect,authority_scope=ApprovalScope(effect_refs=(effect,),authority=decision.spec.effective_authority),valid_from=NOW-timedelta(minutes=1),valid_until=NOW+timedelta(minutes=1),binding_digest=D)
 return ApprovalBinding(apiVersion="servicefabric.ai/v1alpha1",kind="ApprovalBinding",metadata=ResourceMetadata(id="binding-1",name="Binding",description="Fixture.",owner_ref=OwnerReference(kind="service",id="approval")),spec=spec.model_copy(update=changes))
class GovernanceIntegrationTests(unittest.TestCase):
 ...
 def test_approval_binding_requires_exact_unexpired_intent(self):
  ...
  value,runtime=boundary(bundle(approval=("task_create",)),governed_profile);expired=binding_for(value,governed_profile,source,valid_until=NOW);value._approval_lookup=lambda _ref:expired;self.assertEqual(value.invoke(request(approval_refs=("binding-1",)),trusted_adapter_ref="trusted-mcp-adapter",now=NOW).error.code,"SF-APPROVAL-INVALID");self.assertEqual(runtime.calls,[])
  value,runtime=boundary(bundle(approval=("task_create",)),governed_profile);older=binding_for(value,governed_profile,source,policy_decision_ref="policy-old");value._approval_lookup=lambda _ref:older;self.assertEqual(value.invoke(request(approval_refs=("binding-1",)),trusted_adapter_ref="trusted-mcp-adapter",now=NOW).error.code,"SF-APPROVAL-INVALID");self.assertEqual(runtime.calls,[])
  value,runtime=boundary(bundle(approval=("task_create",)),governed_profile);wrong_tenant=binding_for(value,governed_profile,source,tenant_ref="tenant-other");value._approval_lookup=lambda _ref:wrong_tenant;self.assertEqual(value.invoke(request(approval_refs=("binding-1",)),trusted_adapter_ref="trusted-mcp-adapter",now=NOW).error.code,"SF-APPROVAL-INVALID");self.assertEqual(runtime.calls,[])
  for changes in ({"policy_version":"0.9.0"},{"tool_id":"research.search_papers"},{"authority_scope":ApprovalScope(effect_refs=("task-create",),authority=AuthorityGrant(scopes=()))}):
   value,runtime=boundary(bundle(approval=("task_create",)),governed_profile);invalid=binding_for(value,governed_profile,source,**changes);value._approval_lookup=lambda _ref:invalid;self.assertEqual(value.invoke(request(approval_refs=("binding-1",)),trusted_adapter_ref="trusted-mcp-adapter",now=NOW).error.code,"SF-APPROVAL-INVALID");self.assertEqual(runtime.calls,[])
  approvals=ApprovalService();runtime=Runtime();value=GovernedInvocationBoundary(evaluator=VersionedPolicyEvaluator((bundle(approval=("task_create",)),)),approvals=approvals,runtime=runtime,profiles=(governed_profile,));binding=binding_for(value,governed_profile,source);value._approval_lookup=lambda _ref:binding;self.assertEqual(value.invoke(request(approval_refs=("binding-1",)),trusted_adapter_ref="trusted-mcp-adapter",now=NOW).status,"success");self.assertEqual(value.invoke(request(approval_refs=("binding-1",)),trusted_adapter_ref="trusted-mcp-adapter",now=NOW).error.code,"SF-APPROVAL-INVALID")
 def test_constrained_allow_and_durable_acceptance_use_canonical_boundaries(self):
  ...
  value,runtime=boundary(limited,constrained);self.assertEqual(value.invoke(request(),trusted_adapter_ref="trusted-mcp-adapter",now=NOW).status,"success");self.assertEqual(runtime.calls[0].spec.budget.maximum_wall_clock_ms,10);self.assertEqual(len(runtime.calls),1)
  scopes=("math-calculate","unrelated");attenuated=replace(profile(),requested_authority=AuthorityGrant(scopes=scopes,tenant_ref="tenant-demo"));value,runtime=boundary(limited,attenuated);self.assertEqual(value.invoke(request(scopes=scopes),trusted_adapter_ref="trusted-mcp-adapter",now=NOW).status,"success");self.assertEqual(runtime.calls[0].spec.caller_context.scopes,("math-calculate",))
