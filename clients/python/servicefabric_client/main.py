...
class LocalRuntime:
    def __init__(self, home: Path):
        ...
        self.runtime_service = ToolRuntimeService(self.kernel)
        ...
        self.governed = GovernedInvocationBoundary(evaluator=VersionedPolicyEvaluator((bundle,)), approvals=ApprovalService(), runtime=self.runtime_service, profiles=(profile,))
        repo = Path(__file__).resolve().parents[3]
        self.applications = create_application_builder_service(
            portfolio_root=repo / "portfolio" / "applications",
...
def dispatch(argv: list[str]) -> tuple[int, object]:
    ...
    if args.command == "apps":
        ...
        request_id = f"local-build-{args.application_id.replace('.', '-')}-{args.revision.replace('.', '-')}"
        request = {
        ...
        return 0, runtime.applications.build_application(
            ApplicationBuildRequest.model_validate(request)
        )
